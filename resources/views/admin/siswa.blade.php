                        ...
                        <tbody>
                            @foreach ($data as $index => $row )
                                <tr>
                                    ...
                                    <td style="text-align: center; word-break: break-word; white-space: normal; overflow-wrap: break-word; max-width: 100px;">{{ \Carbon\Carbon::parse($row->tgllahir)->format('d-m-Y') }}
                                    <td style="text-align: center">{{ $row->status}}</td>
                                    <td style="text-align: center;">
                                        <a href="{{ route('siswa.edit', $row->noinduk) }}" class="btn btn-info btn-sm">Edit</a>
                                        <form action="{{ route('siswa.destroy', $row->noinduk) }}" method="POST" style="display: inline">
                                          @csrf
                                          @method('DELETE')
